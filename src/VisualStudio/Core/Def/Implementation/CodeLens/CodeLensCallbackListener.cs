...
namespace Microsoft.VisualStudio.LanguageServices.CodeLens
{
    ...
    internal class CodeLensCallbackListener : ICodeLensCallbackListener, ICodeLensContext
    {
        ...
        private Task<Document?> GetDocumentAsync(Solution solution, Guid projectGuid, string filePath, CodeLensDescriptorContext descriptorContext)
        {
            ...
            if (descriptorContext.Properties.TryGetValue("RoslynDocumentIdGuid", out var documentIdStringUntyped) &&
                documentIdStringUntyped is string documentIdString &&
                Guid.TryParse(documentIdString, out var documentIdGuid) &&
                descriptorContext.Properties.TryGetValue("RoslynProjectIdGuid", out var projectIdStringUntyped) &&
                projectIdStringUntyped is string projectIdString &&
                Guid.TryParse(projectIdString, out var projectIdGuid))
            {
