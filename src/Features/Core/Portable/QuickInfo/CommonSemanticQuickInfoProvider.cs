...
namespace Microsoft.CodeAnalysis.QuickInfo
{
    internal abstract partial class CommonSemanticQuickInfoProvider : CommonQuickInfoProvider
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (tokenInformation.ShowAwaitReturn && (tokenInformation.Symbols.First() as INamedTypeSymbol)?.SpecialType == SpecialType.System_Void)
            {
        ...
        private static bool IsOk([NotNullWhen(returnValue: true)] ISymbol? symbol)
        {
            ...
            if (symbol is ITypeParameterSymbol typeParameter && typeParameter.TypeParameterKind == TypeParameterKind.Cref)
                return false;
